...
import { providerJsonFetch } from './backendApi'
...
const AGENT_IMAGE_INSTRUCTIONS = [
  ...
  '',
  '## Progressive Queue Generation',
  'For image requests, call generate_image_batch with one or more image prompts. The app queues each image as a backend generation job.',
  'For multi-image requests, use a progressive batching strategy to ensure consistency:',
  '  1. **Base Reference First:** If the images need to share a consistent style, character, or layout (e.g. PPT slides, storyboards), queue ONE primary image first to establish the visual baseline, then call continue_generation only after it is available in a later turn.',
  '  2. **Batch Remaining Tasks:** Once the base reference is available, list all remaining images to be generated. The app will queue them for backend execution. In your descriptions, explicitly instruct to reference the base image to maintain consistency.',
  '  3. **Independent Images:** If the requested images are completely independent (e.g. "3 different cats"), include them together in ONE generate_image_batch call. Do NOT queue them one by one across multiple responses.',
  ...
  '',
  '## Generating images',
  '- One generate_image_batch item per distinct image. Never collage.',
  '- Dependent images (a later image needs to reference an earlier one) → queue the prerequisite first, then wait for a later user turn or call continue_generation only when the prerequisite is available. The next round will have the result available as `<ref id="..." />`.',
...
function createAgentTools(params: TaskParams, profile: ApiProfile, settings: AppSettings, maskDataUrl?: string): Array<Record<string, unknown>> {
  ...
  tools.push({
    ...
    description: [
      'Queue one or more backend image generation jobs.',
      'Use this for single-image requests, prerequisite/base images, and independent multi-image batches.',
...
export async function callAgentConversationTitleApi(opts: {
  ...
  try {
    ...
    const response = await providerJsonFetch(profile, 'responses', {
      model: profile.model || settings.model,
