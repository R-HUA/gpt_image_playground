...
function isBackendQueuedGenerationTask(task: TaskRecord) {
  return typeof task.queuedAt === 'number'
}
...
export function markInterruptedOpenAIRunningTasks(tasks: TaskRecord[], now = Date.now()) {
  ...
  const updatedTasks = tasks.map((task) => {
    if (!isRunningOpenAITask(task) || task.customTaskId || isBackendQueuedGenerationTask(task)) return task
...
async function executeAgentRound(
  ...
  try {
    ...
    showTaskCompletionNotification(
      queuedCount > 0 ? 'Agent 已提交图像任务' : outputIds.length > 0 ? 'Agent 已生成图片' : 'Agent 已回复',
      queuedCount > 0 ? `Agent 回复已结束，${queuedCount} 个图像任务已进入队列。` : outputIds.length > 0 ? `Agent 回复已结束，共生成 ${outputIds.length} 张图片。` : 'Agent 回复已结束。',
    )
  } catch (err) {
